...
public class ClientFilter2 implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        if (token != null && !"".equals(token)) {
            String CLIENTSESSIONID = verifyToken(token);
            if (CLIENTSESSIONID != null) {
                Cookie cookie = new Cookie("CLIENTSESSIONID", CLIENTSESSIONID);
//                Cookie user = new Cookie("userName", tokenMap.get(CLIENTSESSIONID).getUser().getName());
//                response.addCookie(user);
        ...
        String[] reqFilter = {"/bookcase/user"};
